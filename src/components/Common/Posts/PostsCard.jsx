...
const PostsCard = ({ post }) => {
  const { details, created, postImg, id: postId, userId, username } = post;
    ...
    <section>
      ...
      <div className="flex items-center justify-between w-full md:w-[70%] mt-[2rem] md:mt-0">
        <p className="text-xs text-gray-600">
          {readTime({ __html: details.desc })} min read .
          {moment(created).format("MMM DD")}
        </p>
        <div className="flex items-center gap-3">
          {currentUser?.uid === userId && (
            <Actions postId={postId} title={details.title} desc={details.desc} />
          )}
        </div>
      </div>
    </section>
